...
define(['exports',
        ...
        'atum/interpret',
        'atum/compute/tail',
        'sheut/semantics/debug/debuggable',
...
Debugger.create = function(p, ctx, ok, err) {
    ...
    return new Debugger(null, function() { return interpret.exec(p, ctx, pok, perr); }, ctx, false);
};
...
Debugger.prototype.getFrom = function(p, ok, err) {
    return this.run(
        value_reference.getFrom(
            internal_reference.getFrom(p)),
        ok,
        err);
};

Debugger.prototype.getValue = function(val, ok, err) {
    return this.getFrom(compute.just(val), ok, err);
};
